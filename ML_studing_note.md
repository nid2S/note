...
- 역전파 > 순전파(평범하게 가설을 이용해 예측값 도출)와의 오차(손실, 비용)를 계산한 뒤 파라미터를 갱신하는 과정. 
  출력층에서부터 이전층으로 이동하며(역전파 1단계 > 2단계)해당 단계의 파라미터를 갱신. 
...
- 가중치 감쇄(Weight decay) : 과적합을 막기 위해 함수의 복잡도를 조정할 수 있게 하는 기법. 제곱 놈 정규화(squared norm regularization)등이 사용됨.
- 데이터 증강(data augmentation) : 데이터를 부풀려 성능을 좋게 만듦. (ex - 이미지:반전/자름/밝기조절 등)
...
- Sigmoid : 입력을 전부 0~1의 미분가능한 수로 변환. 이진 분류의 출력층에서 주로 사용. |  1/(1+np.exp(-x)). 가중치는 그래프의 경사도를, 절편은 좌우이동을 결정. |
  시그모이드에서 기울기가 0에 가까운 양 극은 기울기와 같은 값이 나오게 되는데, 이를 곱하면 기울기 소실 문제(앞에 기울기 잘 전달 X)가 발생.    
