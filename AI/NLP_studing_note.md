...
- 셀프 어텐션 실행 : 이를 이용해 스코어(q*k/√(k벡터 차원), 트랜스포머는 Scaled dot-product Attention을 사용)를 구한 뒤 softmax를 지나 어텐션 분포를 구하고, 
  이를 가중합해 어텐션값을 구함. √(k벡터 차원)으로 나누는 이유는 더 안정적인 gradient를 가지게 하기 위함임.
...
- 포지셔널 인코딩 : 각 단어의 임베딩 벡터에 위치 정보를 더해 입력으로 사용, 임베딩벡터가 모인 문장벡터행렬과 포지셔널인코딩 벡터행렬 간의 덧셈을 통해, 
  같은 단어라도 위치에 따라 입력으로 들어가는 임베딩벡터의 값이 달라지게함. 이 벡터는 모델이 학습하는 특정한 패턴을 따르는데, 이는 각 단어의 위치와 시퀀스 내의 다른 단어간의 위치차이에 대한 정보를 알 수 있게 해줌.
  이 값들을 단어들에 임베딩에 추가하면 Q,K,V벡터들로 나중에 투영되었을 때 단어간의 거리를 늘릴 수 있다는 장점이 있음.
