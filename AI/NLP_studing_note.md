...
- 개체명 인식 : 각 단어의 유형이 사람, 장소, 단체등의 유형인지 알아냄. 주어진 유형중 단어가 어떤 유형인지 예측하며, 모델에 따라선 품사 정보를 입력으로 요구하기도 함. 
...
- 셀프 어텐션 실행 : 이를 이용해 스코어(q*k/√(k벡터 차원), 트랜스포머는 Scaled dot-product Attention을 사용)를 구한 뒤 softmax를 지나 어텐션 분포를 구하고, 이를 가중합해 어텐션값을 구함. 
  √(k벡터 차원)으로 나누는 이유는 더 안정적인 gradient를 가지게 하기 위함이며, 논문상에서는 각 가중치 벡터들이 입력 벡터의 차원보다 더 작지만 꼭 그래야 하는 것은 아니고, 단지 멀티헤드에텐션의 계산복잡도를 일정하기 하기 위함임.
...
- 셀프 어텐션 행렬연산 과정 : 문장행렬에 가중치행렬을 곱해 Q,K,V 행렬을 구하고, Q와 K를 내적곱(Q*K^t)하고 (q\*k/√(k벡터 차원))로 나눠 어텐션스코어를 얻으며, 
  여기에 softmax를 지나게 하고(어텐션 분포) V행렬을 곱해 어텐션값 행렬을 만들 수 있음. `softmax((Q * K.T())/sqrt(k_dim)) * V`의 식으로 표현될 수 있음.
...
- 주요 성질 : 각 위치의 단어가 그만의 path를 통해 인코더에서 흘러감. Self-attention층에선 이 위치에 따른 path간의 의존성(depencdency)이 존재하나, FF층은 의존성이 없어 다양한 path들이 병렬처리 될 수 있음.
...
- 포지셔널 인코딩 : 각 단어의 임베딩 벡터에 위치 정보를 더해 입력으로 사용, 임베딩벡터가 모인 문장벡터행렬과 포지셔널인코딩 벡터행렬 간의 덧셈을 통해, 같은 단어라도 위치에 따라 입력으로 들어가는 임베딩벡터의 값이 달라지게함. 
  이 벡터는 모델이 학습하는 특정한 패턴을 따르는데, 이는 각 단어의 위치와 시퀀스 내의 다른 단어간의 위치차이에 대한 정보를 알 수 있게 해줌. Q,K,V벡터들로 나중에 투영되었을 때 단어간의 거리를 늘릴 수 있다는 장점이 있음. 
  임베딩 벡터의 각 값들을 1 ~ -1 의 값을 가짐. 상대적인 위치정보를 사용할 수 있는건 sin(x+y)와 cos(x+y)가 y, sin(x), cos(x)내에서 표현될 수 있기 때문임.
- 포지셔널 인코딩 함수 : 기하학적 시간척도(timescales)를 사용하늗데, 각각의 timescales는 channel/2와 동일하며 이를 사용해 sin(timestep/timescale)과 cos(timestep/timescale)를 구해 channel차원에서 연결함.
  `concat([sin(scaled_time), cos(scaled_time)], axis=1)`의 식으로 구해지며, 이후 pad(`tf.pad(signal, [[0, 0], [0, tf.mod(channels, 2)]])`)와 reshape(`tf.reshape(signal, [1, length, channels])`)를 거침.
  시간은 위치(tf.range(length))와 `min_timescale * exp(tf.range(channels//2, dtype=float) * - log(max_timescale / min_timescale) / (channels//2 - 1.0))`으로 구해지는 inv_timescale의 expand(0|1)의 곱으로 구해짐.
  이 외의 다른 방법(예를들어 f(pos/10000^(i/트랜스포머 출력차원)), f는 임베딩벡터 내 인덱스(pos, i의 i)가 홀수면 sin, 짝수면 cos)도 존재하나 이것이 본적없는 길이의 시퀀스에서도 포지셔널인코딩을 생성할 수 있어 확장성에서 큰 이점을 가짐.
...
- 멀티헤드 어텐션 : d_model 차원을 num_heads로 나눠, d_model/num_heads의 차원을 가지는 Q,K,V에 대해 병렬 어텐션을 수행. 각 어텐션값 행렬을 어텐션헤드 라고 함. 
  다른시각으로 정보의 수집이 가능함. 모든 어텐션 헤드의 결과를 연결하고, 그에 또다른 가중치행렬을 곱해 FF Layer에 보내는 것으로 최종결과물을 만들어냄.
...
- 잔차 연결(residual connection) : 인코더/디코더의 각 층의 서브층의 연결에 추가적으로 사용되는 기법. 서브층(인코더/디코더의 세부 구성 층)의 입력과 출력을 더해 모델의 학습을 도움.
- 층 정규화 : 인코더/디코더의 각 층의 서브층의 연결에 추가적으로 사용. 텐서의 마지막 차원에 대해 평균과 분산을 구하고, 이를 이용해 값을 정규화 해 학습을 도움.

- 인코더 : 레이어층의 개수(하이퍼파라미터) 만큼 인코더층을 쌓음. 인코더층은 크게 두개의 서브층(셀프어텐션, 피드포워드)으로 구성. 

- 디코더 : 인코더 마지막층의 출력과 지난 디코더의 출력을 각 디코더층 연산에 사용. 교사강요 사용(룩-어헤드 마스크 사용). 새개의 서브층(멀티헤드 셀프 어텐션/인코더-디코더 어텐션/피드포워드)으로 구성.
  EOS토큰을 예측할 때 까지 반복됨.
...
- Loss : Crossentropy사용. 단, 이때 확률이 가장 높은 하나의 단어만 저장할 수 도 있지만(GreedyDecoding)BeamSearch라고 불리는 것을 사용할 수도 있음.
- BeamSearch : 가장 확률이 높은 몇개의 단어를 저장하고, 그 수만큼 모델을 돌려 다음 단어를 생성해 보아 모든 시퀀스를 고려했을때 더 낮은 에러를 보이는 결과의 첫번째 출력이 실제 출력으로 선택됨.
  고려할 단어의 수나 미래 출력의 개수는 마음대로 정할 수 있고, 이를 마지막 스텝까지 반복함.
