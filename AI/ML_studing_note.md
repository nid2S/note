...
- 머신러닝은 최적화와 일반화를 잘 조절해야 함. 100%에 가까운 모델을 만들고, 그 후 일반화를 진행. 데이터 사이즈가 같을때 정확도를 높이려면 층추가/크기증가/epoch높이기를, 속도는 배치를 줄이면 됨. 
- accuracy와 동일하게 loss도 절대적인 지표가 되지 않음. 예를들어, loss가 낮다면 너무 학습이 잘 되서 노이즈마저 학습하는 경우 일 수 있음.
...
#### 학습 스케줄링
- 학습률을 너무 크게 잡으면 발산할 수 있고, 작게 잡으면 최적점에 수렴하는데 너무 오래걸려, 큰 학습률에서 시작해 점점 낮추면 좋은 솔루션을 빨리 발견 가능해 나오게 되었음.
