...
- FLOPs(Floating Point Operations per second)Utilization : (?). 언어모델의 학습 효율성을 측정하기 위한 지표.
...
***
- 활성화 함수 : 입력을 받아 수학적 변환을 수행해 출력을 생성하는 함수. 입력값이 특정 뉴런에서 처이되어 결과값을 생성할 때 적용되는 함수.
  각 뉴련계층의 값을 그 다음 뉴런으로 전달할 때 비선형성을 부여해주게 됨. 
- 활성화 함수 특징 : 비선형 함수여야함(선형의 경우 은닉층을 연속으로 추가한것과 1회 추가한것의 차이가 없음), 미분가능해야 함(손실값의 최소화를 위한 경사하강법을 위해).
...
- Sigmoid : 입력을 전부 0~1의 미분가능한 수로 변환. 이진 분류의 출력층에서 주로 사용. |  1/(1+np.exp(-x)) | 항상 양수를 출력해 `입력가중치 합 < 출력 가중치 합`의 가능성이 커(분산이 커짐)기울기소실의 가능성이 높음.
  시그모이드 함수의 출력값이 0 또는 1에 가까워지면(기울기가 0에 가까워지면)시그모이드 함수의 앞 레이어 기울기가 거의 0이 되어, 이전 레이어의 학습 파라미터가 0에 가까운 기울기를 갖게되고, 결과적으로 학습되지 않는 죽은 뉴런이 되어버려 현재는 잘 활성화함수로 잘 쓰이지 않음.
- tanh(Hyperbolic Tangent) : 입력을 -1~1의 미분 가능한 수로 변환. 시그모이드의 대체제. 시그모이드와 함께 출력이 양 극에 가까워 졌을 때 Vanishing gradient problem(기울기 소실 문제)을
- 가지고 있으나 조금 더 나은 편(0 중심이라 반환값의 변화폭이 더 큼). 대표적으로 RNN에서 활성화함수로 사용됨. | (2/1+np.exp(-2x)) - 1 |
- softmax : 입력을 전부 0~1사이로 정규화. 출력의 총합이 1. 다중 분류의 출력층에서 주로 사용. 0중심 함수. 모든 값이 0보다는 조금 큰 값을 가지도록 하는 성질이 있음. |  np.exp(x) / np.sum(np.exp(x))
...
#### GAN
- GAN(Generative Adversarial Network) : 생성적 적대 신경망. 생성기(Generator)와 판별기(Discriminator)라고 불리는 두개의 심층 신경망을 학습시키고 서로 경쟁시켜 훈련하는 비지도학습 신경망.
- 응용 : 판별기가 이용할 수 있는 label데이터가 제한된다 하더라도 대상 이미지에 대한 주요 피처를 학습할 수 있어, label데이터가 부족할 때 판별기를 만들고, 이것의 피처를 추출해 이를 근간으로 새로운 분류기를 만들 수 도 있음(이진분류 -> 다중분류).

- 생성기 네트워크 : 입력으로 고정된 차원의 랜덤 벡터를 사용해 위조 이미지를 만듦. 학습과정을 통해 이미지와 더 비슷한 가짜 이미지를 만들게 됨.
  고정된 잠재벡터를 어떤 형상의 이미지에 대응시킴.
- 생성기 자세히 : 들어온 랜덤 벡터에 전치 컨볼루션, 배치 정규화, ReLU를 적용해 필요 크기의 이미지를 생성. 마지막 레이어는 tanh로, 생성하는 값의 크기를 한정시킴.
  학습 과정은[판별기 학습(실제) -> 판별기 학습(가짜) -> 판별기 최적화 -> 판별기 결과(가짜)와 진짜 레이블간 오차 계산(피드백 기반 학습) -> 생성기 최적화]의 과정을 거침.
- 전치 컨볼루션(부분 스트라이드 컨볼루션) : 일반적인 컨볼루션이 동작하는 방식과 반대방식으로 동작. 입력벡터를 더 높은 차원에 매핑하는 방법을 계산하려 함. 벡터를 필요한 차원의 텐서로 변환 후, 역전파를 통해 커널값을 조정할 수 있음.

- 판별기 네트워크 : 이미지를 입력하고 그 이미지가 실제인지 가짜인지를 예측함. 실제 이미지가 입력될수도, 가짜 이미지가 입력될수도 있음. 학습과정을 통해 실제와 가짜를 더 잘 식별하게 됨.
  생성기가 만든 네트워크 또는 실제 이미지를 입력받아 실제인지 가짜인지 평가점수를 출력함.
- 판별기 자세히 : 들어온 이미지의 진짜 여부를 판별하는 이진분류. 활성화 함수로 LeakyLeLU를, 마지막 레이어로 sigmoid를 사용하는데, 마지막 레이어에서[Dense -> sigmoid]가 아닌[Conv(전역 평균 풀링) -> sigmoid]로 사용됨.
  오차 함수는[maximize log(D(x)) + log(1-D(G(z)))]로, 실제 이미지와 생성기 네트워크에 의해 생성된 이미지로 총 두번의 역전파를 거쳐 학습됨.

- DCGAN(Deep Convolution) : 스스로 학습하고 의미있는 이미지를 생성하는 방법을 소개한 GAN초기 모델 중 하나. Convolution필터만 사용하고 Max Pooling은 사용하지 않음. 안정성 분제를 조금이나마 해결할 수 있음.
...
#### 오토인코더(AutoEncoder)
- 오토인코더 : 비지도 학습. 입력을 출력으로 복사하는 신경망. 항상 인코더와 디코더로 되어있고, 입력과 출력의 뉴런수가 동일함. 여러 제약을 주어 바로 복사하지 못하게 방지하며, 데이터의 효율적 표현 방법을 학습함.
...
- transfer learning(전이학습) : 기존에 잘 훈련된 모델을 사용해 유사한 문제를 가지는 새로운 모델을 학습하는 방법. 다른 유즈케이스에 대해 훈련된 모델 가중치를 사용하는 학습 형태.
...
- 세부 사용 : 사전 학습된 레이어의 가중치는 많은 주요 특성을 인식하도록 훈련되어 있으므로, 다른 레이어의 가중치는 동결하고 마지막 classifier의 가중치만 미세조정을 함.
  lr은 최대 1e-3정도로 설정하고, 옵티마이저에 classifier의 파라미터만 전달하는 식으로 진행됨. 여기서 dropout을 바꾸거나, 데이터 증식 등의 기법을 사용해 조금씩 성능을 더 높일 수 있음.
- 사전 계산된 feature사용 : 다른 레이어의 가중치가 고정 == 각 epoch마다 레이어의 출력값은 고정됨 => 그럼 미리 계산해서 그걸로 돌리면 되지 않나? 를 통해 사용되는 기법. 
  미리 데이터를 계산한 뒤 해당 데이터롤 이용해 classifier만 학습 루프를 진행. 학습속도를 향상시킴.

#### ResNet
- ResNet : 네트워크 레이어에 바로가기 연결을 추가하고 잔여 매핑을 명시적으로 지정해 레이어 수로 인한 부작용을 해결함. 많은 파라미터가 필요한 Dense의 사용을 최대한 지양하기에, 타 모델에 비해 파라미터가 적음. 
  x에서 H(x)를 학습하는게 아닌 두 레이어간의 차이를 학습함. 직접 H(x)를 계산하려면 F(x)+x를 학습하면 됨.
- 구성 : 일련의 레이어와 블록 입력에서 블록 출력으로 추가하는 바로가기 연결로 구성됨. 

#### Inception
- 인셉션 : 크기가 다른 여러 필터로 컨볼루션 연산을 수행하고, 모든 출력을 연결함. 1x1, 3x3, 5x5, 3x3 MaxPooling등을 각각 따로 적용한 뒤 출력을 결합해 출력함.
  주로 3x3, 5x5 컨볼루션에 통과시키기 전 1x1 컨볼루션을 먼저 적용하는 변형이 일반적으로 사용됨. 1x1블록은 차원축소의 역할을 담당하며, 이는 계산량의 병목 문제를 해결하는데 유용함.  

#### DenseNet
- DenseNet : 각 레이어가 그 다음 모든 레이어에 연결되어있는 네트워크. 특정 레이어는 앞에 위치하는 모든 레이어의 피처맵을 입력받게 됨.
...
## Tasks
### StyleTransfer
- StyleTransfer : 콘텐츠 이미지와 스타일 이미지를 제공한 뒤, 콘텐츠 이미지로부터 내용을 가져오고, 스타일 이미지로부터 스타일을 가져와 새로운 이미지를 제작.

- 오차 : 콘텐츠 오차와 스타일 오차 두가지를 사용.
- 콘텐츠 오차 : CNN이 이미지를 학습할 때, 초기 레이어는 선, 곡선 및 모양과 같은 일반적인 정보를 학습하고 뒷 레이어는 눈, 건물 및 나무 등의 상위 개념을 학습함. 
  따라서, 비슷한 이미지의 마지막 레이어는 상당히 비슷한 경향을 보이고, 이를 콘텐츠 오차에 적용. 마지막 레이어에서 콘텐츠 이미지와 생성된 이미지의 값을 MSE를 적용해 오차를 계산함.
- 스타일 오차 : 유사한 스타일이 적용된 이미지는 그램매트릭스에 대해 비슷한 값을 가진다는 것을 이용, 두 이미지의 그램매트릭스 값의 MSE를 적용해 오차를 계산함.

- GramMatrix : 이미지 스타일의 캡쳐 방법. 여러 레이어에서 캡쳐된 피처맵의 상관관계를 계산해 스타일을 계산하는 방법을 제공함. 피처의 상관관계 값. 
  모든 이미지를 (channel, h*w)로 만들어 벡터화 한 뒤 그 벡터의 전치와 곱하고, 이 값을 요소 수로 나눠 정규화(특정 피처맵이 주도적으로 영향을 끼치는걸 막음)해 값을 얻을 수 있음. 

## ONNX
- ONNX(Open Neural Network Exchange) : Tensorflow, Pytorch등 서로 다른 DNN프레임워크 환경에서 만들어진 모델들을 서로 호환되게 사용할 수 있도록 만들어진 공유 플랫폼. 딥러닝 모델의 오픈소스 포맷.
  이 오픈소스 포맷은 기본 연산자, 표준 데이터 형식과 확장가능 계산 그래프 모델을 정의하며, 순전파시의 함수/호출들에 최적화된 그래프인 trace/script를 생성해 변환함. 파이썬 기반 프레임워크로 개발된 딥러닝 모델을 C++기반의 Caffe2로 변환할 수 있음.
- 장점 : Framework Interoperability(상호운용성)(특정 환경에서 생성된 모델을 다른 환경으로 import해 자유롭게 사용가능, 개발 효율성이 높은 프레임워크로 개발한 뒤 운영효율성이 높은 모델로 변환해 운용할 수 있음)과
