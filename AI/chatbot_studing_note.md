...
- Human-in-the-loop(HIL) : 학습된 모델을 사람이 피드백을 주면서 점진적으로 개선시키는 방법. active Learning이라고도 불림.
...
- 언어폭력 데이터 : 일단 양 자체가 적기도 하며, 은어/신조어가 많이 섞여있고, 비꼬는 언어도 많이 사용되며, 긍정적인 단어를 사용하는 성차별(Ambivalent Sexism)과 같이 
  화자의 의도를 파악하기 힘든 경우도 많으며, 적은량의 데이터만 가지고 학습될 경우는 의도치 않은 편향성(여성명사만 나오면 성차별로 인식하는 등)울 가질 수 도 있어 조심해야 함.


## 대화형 시스템 아키텍처
- 참고 : [블로그](https://medium.com/analytics-vidhya/architecture-of-a-conversational-ai-system-5-essential-building-blocks-2f149466da1d)
- 부분 분류 : 사용자 인터페이스, AI기술, 대화디자인, 벡엔드 통합 및 분석의 최소 5개의 부분으로 볼 수 있음.
- 사용자 인터페이스 : 대화 경험의 포털. 웹사이트의 채팅 위젯, FacebookMassenger, Slack등 메시징 앱, 스마트스피커, 전화, SMS 전부 포함됨.
  실제 인터페이스를 디자인하는 프로세스인 UI디자인을 통해 디자인되는데, 이때 사용할 수 있는 여러 규칙(시각적 요소 -> 게슈탈트 | 기능적 UI -> Shneiderman 의 Golder | UX -> Hick의 법칙)이 존재함.
- AI 기술 : 대화형 AI솔루션 설계의 핵심구성요소. 엔진을 디자인함. 시스템이 사용자의 입력발언을 인식하고, 주어진 컨텍스트에서 의도를 이해하고, 조치를 취하고, 적절히 대응하도록 하는
  필요 기술 솔루션을 설계하는것부터 모든 것과 관련되어있음. 대화 컨텍스트를 유지하는데 필요한 기술도 포함되어, breakdown발생시 AI비서나 사용자가 이를 복구할 수 있음. UI모델은 일반적으로 백엔드의 AI모듈에 연결됨.
- 대화디자인 : 엔진을 구동할 연료를 디자인하고 개발함. 사용자와 챗봇간의 실제 대화 여정을 다룸. 대화의 흐름, 패턴, 대화가 깨질 시의 대응 등 패턴/예외규칙 및 상호작용요소를 디자인함.
- 백엔드 통합 : 대화형 AI비서를 기존 IT시스템과 연결 및 상호작용을 하게 함. 잘 설계된 백엔드 통합은 AI도우미를 더 지식이 풍부하고 유능하게 만듦. 
- 분석 설계 : 대화를 기록하기 위한 솔루션, 통찰력 추출, 결과 시각화, 모델 모니터링, 재교육을 위한 재샘플링등이 포함됨. 지속적인 모니터링과 보정을 위함. 
