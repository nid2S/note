...
### Decoding algorithm
- 디코딩 알고리즘 : 생성대화모델에서 생성된 답변을 디코딩 할 때 사용하는, 성능의 향상과 더 좋은 답변을 위한 알고리즘. 

- BeamSearch : greedy search(최대확률)와 함께 널리 사용되는 결정적 디코딩접근중 하나. greedy search는 이것의 특별한 경우라고 볼 수 있음. 빔 사이즈를 바꿔가며 실험해봐야 할 필요성이 있음.
- Sampling : 어떤 자료에서 일부 값을 추출하는 것을 의미. 각 단계의 모델종속분포에서 샘플링. 낮은 가능성의 토큰을 샘플링하는걸 막기 위한 전형적 접근법은 각 단계에서 vocab의 하위집합으로 샘플링을 제한하고,
...
### Metrics
- Perplexity : 언어모델의 내부 성능평가지표. 모델만으로 바로 비교할 수 있는 척도. 평균적으로 아무단어에나 뒤따라 나타날 수 있는/고민하고 있는 단어의 수. 문장의 길이로 정규화된 문장확률의 역수.
  각 단어들의 확률을 전부 곱한 다음 -(1/N(단어개수))승을 해 구할 수 있음. 
...
# Language Model
- Language Modeling : 특정 시점까지의 단어들의 시퀀스가 주어졌을 때, 이후 시점의 단어를 예측하는 task.
- Conditional Language Modeling : 특정 시점까지의 단어들의 시퀀스 뿐 아니라 또다른 입력 x를 받아  P(yt∣y1, ..., yt−1, x)의 분포를 출력하는 task.
  기계번역(x = 원문장, y = 번역문장), 요약(x = 입력문장, y = 요약문장), 대화(x = 대화로그, y = 다음대화)등이 속함(입력 문장에 대한 출력문장이 있는 경우).
